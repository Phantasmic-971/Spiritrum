    ...
    public class Phantasmo : ModItem
    {
        ...
        public override bool? UseItem(Player player)
        {
            if (Main.rand.NextFloat() < 0.03f)
            {
                ...
                {                    SoundEngine.PlaySound(new SoundStyle("Spiritrum/Content/Items/PhantasmoKill", SoundType.Sound) { Volume = 3f }, player.Center);
                }
            ...
            else
            {
                ...
                if (Main.myPlayer == player.whoAmI)
                {
                    SoundEngine.PlaySound(new SoundStyle("Spiritrum/Content/Items/PhantasmoHeal", SoundType.Sound) { Volume = 3f }, player.Center);
                }
            }
            return true;
        }
